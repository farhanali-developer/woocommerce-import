...
foreach($csv_file as $csv_data){
    ...
    if (!empty($csv_file_data)):
        ...
        foreach ($csv_file_data as $product){
            ...
            $parent_id = wc_get_product_id_by_sku($product[$data['Parent']]);
            ...
            if (!$product_id && !$parent_id){
                try {
                    ...
                    if(!empty($product[$data['Categories']])){
                        ...
                        $separate_by_less_than = ">";
                        $separate_by_comma = ",";
                        $separate_by_vertical_line = "|";

                        //Checking if a category is hierarchical
                        if (strpos($product[$data['Categories']], $separate_by_less_than) !== false) {
                            $categories_array = explode('>', $product[$data['Categories']]);
                        }
                        //Check if have multiple categories
                        if (strpos($product[$data['Categories']], $separate_by_comma) !== false) {
                            $categories_array = explode(',', $product[$data['Categories']]);
                        }
                        //Check if have multiple categories
                        if (strpos($product[$data['Categories']], $separate_by_vertical_line) !== false) {
                            $categories_array = explode('|', $product[$data['Categories']]);
                        }
                        ...
                        foreach($categories_array as $cat){
                            ...
                            $category_ID = get_category_id($parent_name, $child_name);
                            $categories[] = $category_ID;
                        }
                        $products->set_category_ids( $categories );
                    }
            ...
            //product found
                ...
                // echo "ELSE: New Product ID is: $product_id \n";
                // echo "ELSE: Parent ID is: $parent_id \n";
                try {
                    $update_product = wc_get_product($product_id);
                    $update_parent_product = wc_get_product($parent_id);
                    // echo "Existing Product ID is: ".$update_product->get_id()." \n";
                    if ( $update_product instanceof WC_Product || $update_parent_product instanceof WC_Product) {
                        ...
                        if($product[$data['Type']] !== "variation" && $product[$data['Type']] !== "variation, downloadable, virtual" && $product[$data['Type']] !== "variation, downloadable" && $product[$data['Type']] !== "variation, virtual"){
                            ...
                            if(!empty($product[$data['Categories']])){
                                ...
                                $update_product->set_category_ids( $categories );
                            }
                            
                            //Set Tags
                            if(!empty($product[$data['Categories']])){
                                $tags_array = explode(',', $product[$data['Categories']]);
                                $tags = array();
                                foreach($tags_array as $tag){
                                    $tag_ID = get_tag_id($tag);
                                    $tags[] = $tag_ID;
                            ...
                            if(!empty($product[$data['Cross-sells']])){
                                $cross_sell_ids = explode(',', $product[$data['Cross-sells']]);
                                $cross_sell_array = array();
                                foreach($cross_sell_ids as $id){
                                    $cross_sell_id = wc_get_product_id_by_sku($id);
                            ...
                            if(!empty($product[$data['Purchase_note']])){
                            ...
                            if($product[$data['Type']] === "simple, virtual"){
                                $update_product->set_virtual( true );
                            }
                            else if($product[$data['Type']] === "simple, downloadable"){
                            ...
                            else if($product[$data['Type']] === "simple, downloadable, virtual" || $product[$data['Type']] !== "simple, downloadable"){
                                $update_product->set_virtual( true );
                                $update_product->set_downloadable( true );
                                $downloads = array();
                                // Creating a download with... yes, WC_Product_Download class
...
// function get_category_id($parent_cat, $cat_name){
...
//         //Check if parent category exists in CSV file
//         if(!empty($parent_cat)){
//             $parent_term = get_term_by('name', $parent_cat, 'product_cat');

//             //Check if parent category exists in WordPress
...
//             //Insert child category if parent found or created
//             $term = wp_insert_term(
//                 $cat_name,
//                 'product_cat',
//                 array(
...
//         //insert category that has no parent in CSV file
//         else{
//             $term = wp_insert_term(
//                 $cat_name,
//                 'product_cat',
...
function get_category_id($parent_name, $child_name){
    if(!empty($parent_name)){
        ...
        $child_term = get_term_by('name', $child_name, 'product_cat');
        if(!empty($child_term)){
